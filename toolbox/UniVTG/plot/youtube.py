...
from toolbox.UniVTG.utils.basic_utils import load_jsonl
from toolbox.UniVTG.utils.temporal_nms import compute_temporal_iou
...
def plot_video(pred_json, save_dir_i, fig_num=None, template=True):
    ...
    if fig_num is None:
        ...
        # fig_num = len(pred_json['pred'])
    fig, axs = plt.subplots(nrows=1, ncols=fig_num, figsize=(40, 20), gridspec_kw={'wspace': 0.0, 'hspace': 0.0})

    vid_exists = os.path.exists(os.path.join(video_path, pred_json['vid'], pred_json['vid'] + '.mp4'))
    assert vid_exists, f"Video {pred_json['vid']} does not exist!"
    if vid_exists:
        cap = cv2.VideoCapture(os.path.join(video_path, pred_json['vid'], pred_json['vid'] + '.mp4'))
        fps = cap.get(cv2.CAP_PROP_FPS)
        for i, t in enumerate(np.linspace( t_min, t_max, fig_num)):
        # for i, frames in enumerate(pred_json['clip']):
            cap.set(cv2.CAP_PROP_POS_MSEC, t * 1e3)
            # cap.set(cv2.CAP_PROP_POS_FRAMES, int(frames[0]+frames[1])/2 )
            rval, frame = cap.read()
...
def plot_hl(pred_json, save_dir_i,  base_json=None):
    ...
    # duration = (pred_json['clip'][-1][0] + pred_json['clip'][-1][1]) / (2 * pred_json['fps'])
    duration = pred_json['clip'][-1][0] / pred_json['fps']
    # t_min, t_max = 0, duration
    # x = np.arange(t_min, t_max, clip_len)
    # x = x[:len(pred_saliency)]
    # x = range(len(pred_saliency))
    x = [i * duration/len(pred_saliency) for i in range(len(pred_saliency))]
    ...
    ax.text(x[-1]-offset, -0.2, f'{x[-1]:.1f}', va='center', ha='center',  color="black", fontproperties=font_prop1)
    for i in x[1:-1]:
...
if __name__ == "__main__":
    ...
    # for domain in ["gymnastics", "parkour", "skating", "skiing", "surfing"]:
    # for domain in ["gymnastics"]:
    for domain in ["skating"]:
        video_path= os.path.join("/Users/kevin/dataset/youtube-highlights/DomainSpecificHighlight-master", domain)

        pred_json_val = load_jsonl(f"/Users/kevin/univtg/plot/youtube/{domain}.jsonl")
        ...
        save_dir = os.path.join(os.path.join('/Users/kevin/univtg/plot', 'youtube', domain))
