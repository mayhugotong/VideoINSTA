...
def plot_sample(sample_id):
    ...
    windows = str(len(gt_json['relevant_windows']))
        
    global gap
    gap = round(gt_json['duration'] / seg_num)
    iou = compute_temporal_iou(pred_json['pred_relevant_windows'][0], gt_json['relevant_windows'][0])
    iou = round(iou, 2)
    save_dir_i = os.path.join(save_dir, '_'.join([str(iou), windows, gt_json['vid'], str(sample_id)]))
...
if __name__ == "__main__":
    ...
    video_path="/Users/kevin/dataset/qvhighlights_val_segment"
    gt_path="./qvhl/gt.jsonl"

    dset_path="/Users/kevin/univtg/results/mr-qvhighlights"
    checkpoint_dir="/Users/kevin/univtg/plot/qvhl"
    json_file="univtg.jsonl"
    
    # baseline
    base_checkpoint_dir = "/Users/kevin/univtg/plot/qvhl"
    base_json_file = "momentdetr.jsonl"
    ...
    # other settings
    template_path = "./plot/settings/template.png"
    color1 = '#90ee90'  # green
    color2 = '#add8e6'  # blue
    ...
    font_path1 = "./plot/settings/calibri.ttf"
    font_path2 = "./plot/settings/calibri-bold.ttf"
    font_path3 = "./plot/settings/calibri-italy.ttf"
    ...
    save_dir = os.path.join(os.path.join('./plot', 'qvhl', checkpoint_dir))
